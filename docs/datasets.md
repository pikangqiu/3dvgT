...
Occ3D-nuScenes and OpenOccupancy are phase-1 external references for public semantic occupancy comparison. They should be treated separately from the repository's local LiDAR-derived binary BEV proxy occupancy:

- local proxy metric: `bev_occupancy_iou`
- public benchmark metrics: semantic occupancy mIoU / class IoU

Prepare the expected root:

```bash
bash scripts/prepare_occ3d.sh
```

The script creates `data/occ3d` by default and prints the required manual download steps. Occ3D/OpenOccupancy labels may require following the dataset author's access and license terms, so the script does not silently download data. If you have an approved archive URL, use:
...
After a model checkpoint exists, attach public labels to the eval manifest, validate public split/path alignment, validate class ids against the expected label space, export BEV occupancy predictions into that benchmark manifest, and then compute class IoU plus semantic occupancy mIoU:
