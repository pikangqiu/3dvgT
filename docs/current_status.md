...
| Occupancy benchmark evaluation | Scripted | `scripts/evaluate_occupancy_benchmark.py` and `src/vggt_project/occupancy_benchmark.py`; computes class IoU and semantic `occupancy_miou` from manifest-paired exported prediction/target arrays while keeping public Occ3D/OpenOccupancy metrics separate from local LiDAR-proxy `bev_occupancy_iou` |
