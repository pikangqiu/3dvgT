...
| Real-run evidence bundle | Scripted | `scripts/verify_real_run_evidence.py` and `src/vggt_project/real_run_evidence.py`; combines a saved real-training preflight JSON, saved artifact verifier JSON, current git commit, optional expected commit, and optional clean-worktree check into one final JSON evidence file |
