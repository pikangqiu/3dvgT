...
| Training readiness | Scripted | `scripts/check_training_readiness.py`; checks config manifests, optional satellite raster config readiness, external adapter/reference-root paths, concrete `.pt`/`.pth`/`.bin` weight file selection, core dependencies, and requested device availability |
