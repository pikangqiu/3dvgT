...
The repository is no longer only a paper/reference folder. It now contains a project scaffold with data contracts, a minimal trainable model scaffold with camera-specific depth and pointmap prediction plus optional BEV occupancy prediction, external adapter module loading, a repo-local G3T/VGGT adapter template, config-level local G3T/VGGT reference adapter selection, config-level reference constructor kwargs, reference-adapter checkpoint loading hooks, configurable fine-tuning policies, model adapter contract checks, manifest forward probing, reconstruction losses, train/eval entrypoints, reconstruction-first metrics, config-driven experiment dispatch and reporting, an end-to-end smoke pipeline, project-status audit scripts, environment/readiness scripts, reproducible environment snapshot reporting, external-asset readiness checks, combined real-training preflight reports, ordered training run-plan generation, combined launch readiness packet generation, dry-run/executable training bootstrap, external-reference setup scripts, model-weight preparation, JSON config weight wiring, weight-download and checkpoint-inspection scripts, dataset preparation notes, satellite raster config setup entrypoint, Occ3D/OpenOccupancy benchmark dataset setup entrypoint, public occupancy label attachment, public occupancy manifest split validation, occupancy label class-id validation, semantic occupancy prediction export, benchmark evaluation for exported arrays, manifest asset materialization, manifest sample preview, satellite raster config validation, multi-camera nuScenes LiDAR-depth target generation and loss/metric wiring, nuScenes ego-frame and camera-frame LiDAR pointmap target generation, LiDAR-derived BEV occupancy proxy target generation, dense G3T/VGGT reference prediction target materialization, combined LiDAR supervision manifest generation, scene-level manifest splitting, GitHub publishing, research baseline notes, and a machine-readable experiment protocol.

It is not yet a complete real nuScenes training codebase. The current train/eval paths are synthetic smoke training/evaluation plus manifest-smoke training that can load real camera, satellite, single- or multi-camera depth, mask, sample-level or camera-level pointmap, LiDAR-derived occupancy proxy, and ego-pose-derived, explicit, calibration-derived, or reference-predicted camera-level pose target tensors from a JSONL manifest. Real training still needs real satellite patch extraction/alignment, actual real public G3T/VGGT checkpoint plus GPU validation evidence, concrete real-checkpoint fine-tuning validation, and a successful real-asset manifest forward probe.
...
| Environment report | Scripted | `scripts/report_environment.py` and `src/vggt_project/environment_report.py`; records Python version/executable, platform, package availability/versions, and torch CUDA/MPS availability for saved real-run evidence |
| Model/device validation | Scripted | `scripts/validate_model_device.py` and `src/vggt_project/model_device_validation.py`; loads the configured model and optional checkpoint onto the requested `cpu`/`cuda`/`mps` device, runs one camera-aware forward pass, and can save `model_device.json`; `--require-weights` blocks real-run evidence when `runtime.model.weights_path` is unset |
...
| External asset readiness | Scripted | `scripts/check_external_assets.py`; checks required real-training assets at a coarse filesystem level: nuScenes layout, satellite raster config, concrete model checkpoint, and optional Occ3D/OpenOccupancy labels, then prints next setup actions and can save `external_assets.json` |
...
| Real-run evidence bundle | Scripted | `scripts/verify_real_run_evidence.py` and `src/vggt_project/real_run_evidence.py`; combines a saved real-training preflight JSON, saved model/device JSON, saved artifact verifier JSON, optional saved environment JSON, current git commit, optional expected commit, and optional clean-worktree check into one final JSON evidence file |
...
| Training run planning | Scripted | `scripts/plan_training_run.py` prints ordered commands from nuScenes manifest generation through satellite crops, camera pose targets, LiDAR supervision, occupancy target generation, optional reference supervision when a G3T/VGGT reference adapter is configured, manifest sample preview, split manifests, train/eval manifest non-empty path validation, optional checkpoint inspection, readiness, adapter contract checks, one-sample manifest forward probe, saved external-asset readiness JSON, saved model/device JSON, saved real-training preflight JSON, train with metrics JSON export, eval with metrics JSON export, optional Occ3D label attachment, public occupancy manifest split validation, occupancy label class-id validation, occupancy prediction export, occupancy benchmark evaluation, artifact verifier JSON export, environment JSON export, and final real-run evidence bundle export; `scripts/bootstrap_training_run.py` can dry-run or execute those commands up to a selected step without skipping launch checks once manifests exist |
...
| Occupancy benchmark export/evaluation | Scripted | `scripts/attach_occ3d_labels.py`, `src/vggt_project/data/occ3d_labels.py`, `scripts/validate_public_occupancy_manifest.py`, `src/vggt_project/public_occupancy_manifest.py`, `scripts/validate_occupancy_labels.py`, `src/vggt_project/occupancy_label_validation.py`, `scripts/export_occupancy_predictions.py`, `src/vggt_project/data/occupancy_predictions.py`, `scripts/evaluate_occupancy_benchmark.py`, and `src/vggt_project/occupancy_benchmark.py`; attaches public Occ3D/OpenOccupancy `labels.npz` paths to eval manifests as `occupancy_path`, validates public split, scene, sample, and label path alignment, validates class ids against the expected label space, exports model BEV occupancy predictions to `predicted_occupancy_path`, then computes class IoU and semantic `occupancy_miou` from manifest-paired prediction/target arrays while keeping public metrics separate from local LiDAR-proxy `bev_occupancy_iou` |
| Training artifact verification | Scripted | `scripts/verify_training_artifacts.py` and `src/vggt_project/training_artifacts.py`; verifies checkpoint existence, train/eval metrics JSON, optional occupancy benchmark JSON, required metrics, occupancy mIoU numeric range, class-IoU numeric ranges, and optional required occupancy class count before a run is recorded in experiment tables |
...
PYTHONPATH=src python3 scripts/check_external_assets.py --help
PYTHONPATH=src python3 scripts/validate_model_device.py --help
PYTHONPATH=src python3 scripts/report_environment.py --help
...
PYTHONPATH=src python3 scripts/verify_real_run_evidence.py --help
PYTHONPATH=src python3 scripts/attach_occ3d_labels.py --help
PYTHONPATH=src python3 scripts/validate_public_occupancy_manifest.py --help
PYTHONPATH=src python3 scripts/validate_occupancy_labels.py --help
...
PYTHONPATH=src python scripts/check_external_assets.py --config configs/reconstruction_first.json --output outputs/manifest-smoke/external_assets.json --json
PYTHONPATH=src python scripts/validate_model_device.py --config configs/reconstruction_first.json --require-weights --output outputs/manifest-smoke/model_device.json --json
PYTHONPATH=src python scripts/report_real_training_preflight.py --config configs/reconstruction_first.json --output outputs/manifest-smoke/real_training_preflight.json --json
PYTHONPATH=src python scripts/verify_training_artifacts.py --checkpoint outputs/manifest-smoke/manifest_smoke_scaffold.pt --train-metrics outputs/manifest-smoke/train_metrics.json --eval-metrics outputs/manifest-smoke/eval_metrics.json --occupancy-report outputs/manifest-smoke/occupancy_benchmark.json --required-eval-metric loss --required-eval-metric depth_mae --required-occupancy-class-count 18 --output outputs/manifest-smoke/training_artifacts.json --json
PYTHONPATH=src python scripts/report_environment.py --output outputs/manifest-smoke/environment.json --json
PYTHONPATH=src python scripts/verify_real_run_evidence.py --preflight-report outputs/manifest-smoke/real_training_preflight.json --artifact-report outputs/manifest-smoke/training_artifacts.json --environment-report outputs/manifest-smoke/environment.json --model-device-report outputs/manifest-smoke/model_device.json --output outputs/manifest-smoke/real_run_evidence.json --require-clean-worktree --expected-git-commit "$(git rev-parse HEAD)" --json
