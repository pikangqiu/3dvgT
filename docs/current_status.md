...
It is not yet a complete real nuScenes training codebase. The current train/eval paths are synthetic smoke training/evaluation plus manifest-smoke training that can load real camera, satellite, single- or multi-camera depth, mask, sample-level or camera-level pointmap, and ego-pose-derived or explicit camera-level pose target tensors from a JSONL manifest. Real training still needs real satellite patch extraction/alignment, dense G3T-style camera-level pointmap target generation, calibrated G3T camera-level pose target generation, replacing the G3T/VGGT adapter template with concrete head calls, and GPU environment validation.
...
| Real nuScenes data loading | Partial scaffold | layout inspection, JSONL manifest generation/loading with ego pose, map location metadata, and optional camera-level pose targets, path validation, smoke satellite/mask asset materialization, local raster satellite config validation/crop materialization, single- or multi-camera LiDAR-projected camera depth target generation, LiDAR-to-ego and LiDAR-to-camera pointmap target generation, real-file smoke tensor loading, optional depth/mask/sample-level pointmap/camera-level pointmap target loading, and ego-pose-derived or explicit camera-level pose targets exist; real satellite raster sourcing, dense G3T-style pointmap target generation, calibrated G3T camera-pose targets, and G3T/VGGT camera-pose adapter wiring still need implementation |
| Model framework | Scaffolded | `src/vggt_project/models/scaffold.py`, `src/vggt_project/models/factory.py`, and `adapters/g3t_vggt_adapter.py`; includes shared BEV/satellite latent heads plus camera-specific depth, pointmap, and local pose heads for manifest batches, factory-based model selection, and a repo-local adapter template |
| G3T/VGGT integration | Partial plumbing | reference code exists; train/eval can load an external adapter module and optional weights through config; `adapters/g3t_vggt_adapter.py` is a smoke-trainable template, but concrete G3T/VGGT head calls are still missing |
...
| Training run planning | Scripted | `scripts/plan_training_run.py`; prints ordered commands from nuScenes manifest generation through satellite crops, LiDAR supervision, manifest sample preview, split manifests, readiness, train, and eval |
