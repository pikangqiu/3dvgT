...
| Real-run evidence bundle | Scripted | `scripts/verify_real_run_evidence.py` and `src/vggt_project/real_run_evidence.py`; combines a saved real-training preflight JSON, saved artifact verifier JSON, optional saved environment JSON, current git commit, optional expected commit, and optional clean-worktree check into one final JSON evidence file |
...
| Training run planning | Scripted | `scripts/plan_training_run.py` prints ordered commands from nuScenes manifest generation through satellite crops, camera pose targets, LiDAR supervision, occupancy target generation, optional reference supervision when a G3T/VGGT reference adapter is configured, manifest sample preview, split manifests, train/eval manifest non-empty path validation, optional checkpoint inspection, readiness, adapter contract checks, one-sample manifest forward probe, saved external-asset readiness JSON, saved real-training preflight JSON, train with metrics JSON export, eval with metrics JSON export, optional Occ3D label attachment, public occupancy manifest split validation, occupancy label class-id validation, occupancy prediction export, occupancy benchmark evaluation, artifact verifier JSON export, environment JSON export, and final real-run evidence bundle export; `scripts/bootstrap_training_run.py` can dry-run or execute those commands up to a selected step without skipping launch checks once manifests exist |
