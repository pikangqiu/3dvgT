...
| Training run planning | Scripted | `scripts/plan_training_run.py` prints ordered commands from nuScenes manifest generation through satellite crops, camera pose targets, LiDAR supervision, occupancy target generation, manifest sample preview, split manifests, readiness, adapter contract checks, train, and eval; `scripts/bootstrap_training_run.py` can dry-run or execute those commands up to a selected step |
