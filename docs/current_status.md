...
| Occupancy benchmark export/evaluation | Scripted | `scripts/export_occupancy_predictions.py`, `src/vggt_project/data/occupancy_predictions.py`, `scripts/evaluate_occupancy_benchmark.py`, and `src/vggt_project/occupancy_benchmark.py`; exports model BEV occupancy predictions to `predicted_occupancy_path`, then computes class IoU and semantic `occupancy_miou` from manifest-paired prediction/target arrays while keeping public Occ3D/OpenOccupancy metrics separate from local LiDAR-proxy `bev_occupancy_iou` |
