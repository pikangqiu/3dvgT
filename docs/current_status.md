...
| Occupancy benchmark export/evaluation | Scripted | `scripts/attach_occ3d_labels.py`, `src/vggt_project/data/occ3d_labels.py`, `scripts/validate_occupancy_labels.py`, `src/vggt_project/occupancy_label_validation.py`, `scripts/export_occupancy_predictions.py`, `src/vggt_project/data/occupancy_predictions.py`, `scripts/evaluate_occupancy_benchmark.py`, and `src/vggt_project/occupancy_benchmark.py`; attaches public Occ3D/OpenOccupancy `labels.npz` paths to eval manifests as `occupancy_path`, validates class ids against the expected label space, exports model BEV occupancy predictions to `predicted_occupancy_path`, then computes class IoU and semantic `occupancy_miou` from manifest-paired prediction/target arrays while keeping public metrics separate from local LiDAR-proxy `bev_occupancy_iou` |
