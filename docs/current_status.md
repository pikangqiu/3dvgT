...
| Model framework | Scaffolded | `src/vggt_project/models/scaffold.py`, `src/vggt_project/models/factory.py`, and `adapters/g3t_vggt_adapter.py`; includes shared BEV/satellite latent heads plus camera-specific depth/pointmap heads for manifest batches, factory-based model selection, and a repo-local adapter template |
