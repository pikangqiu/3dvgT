...
| G3T/VGGT integration | Partial plumbing | reference code exists; train/eval can load an external adapter module and optional weights through config; `adapters/g3t_vggt_adapter.py` is a smoke-trainable template with reference-output mapping for G3T/VGGT-style `depth`, `world_points`, and pose encodings, but concrete G3T/VGGT module instantiation and head calls are still missing |
