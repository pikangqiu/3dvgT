...
It is not yet a complete real nuScenes training codebase. The current train/eval paths are synthetic smoke training/evaluation plus manifest-smoke training that can load real camera, satellite, single- or multi-camera depth, mask, sample-level or camera-level pointmap, LiDAR-derived occupancy proxy, and ego-pose-derived, explicit, calibration-derived, or reference-predicted camera-level pose target tensors from a JSONL manifest. Real training still needs real satellite patch extraction/alignment, real public G3T/VGGT checkpoint and GPU validation, concrete fine-tuning policies for G3T/VGGT heads, and full real-data head-call validation.
...
| Training run planning | Scripted | `scripts/plan_training_run.py`; prints ordered commands from nuScenes manifest generation through satellite crops, camera pose targets, LiDAR supervision, occupancy target generation, manifest sample preview, split manifests, readiness, adapter contract checks, train, and eval |
