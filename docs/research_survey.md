...
- Occ3D/OpenScene/UniOcc provide public occupancy labels and evaluation language; the repository now has Occ3D label attachment plus a lightweight class-IoU/mIoU evaluator for exported arrays, but real public semantic occupancy validation still depends on actual-data split alignment, class mapping, and evaluator-output verification.
