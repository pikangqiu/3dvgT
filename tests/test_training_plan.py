...
class TrainingPlanTest(unittest.TestCase):
    def test_plan_lists_preprocessing_commands_for_missing_manifest_split(self) -> None:
        ...
        self.assertIn("scripts/validate_public_occupancy_manifest.py", rendered)
        self.assertIn("scripts/validate_occupancy_labels.py", rendered)
        ...
        self.assertIn("scripts/check_external_assets.py", rendered)
        self.assertIn("--output outputs/synthetic/external_assets.json --json", rendered)
        self.assertIn("scripts/validate_model_device.py", rendered)
        self.assertIn("--output outputs/synthetic/model_device.json --json", rendered)
        self.assertIn("scripts/report_real_training_preflight.py", rendered)
        self.assertIn("scripts/verify_training_artifacts.py", rendered)
        self.assertIn("scripts/report_environment.py", rendered)
        self.assertIn("scripts/verify_real_run_evidence.py", rendered)
        self.assertIn("--model-device-report outputs/synthetic/model_device.json", rendered)
    ...
    def test_launch_steps_remain_pending_when_required_outputs_exist(self) -> None:
        ...
        self.assertFalse(steps["check_external_assets"].ready)
        self.assertFalse(steps["train"].ready)
        self.assertFalse(steps["evaluate"].ready)
        self.assertFalse(steps["report_real_training_preflight"].ready)
        self.assertFalse(steps["optional_attach_occ3d_labels"].ready)
        self.assertFalse(steps["validate_public_occupancy_manifest"].ready)
        self.assertFalse(steps["validate_occupancy_labels"].ready)
        ...
        self.assertFalse(steps["report_environment"].ready)
        self.assertFalse(steps["verify_real_run_evidence"].ready)
    ...
    def test_plan_validates_split_manifests_before_readiness(self) -> None:
        ...
        self.assertLess(step_names.index("probe_manifest_forward"), step_names.index("check_external_assets"))
        self.assertLess(step_names.index("check_external_assets"), step_names.index("validate_model_device"))
        self.assertLess(step_names.index("validate_model_device"), step_names.index("report_real_training_preflight"))
        self.assertLess(step_names.index("probe_manifest_forward"), step_names.index("train"))
        self.assertLess(step_names.index("probe_manifest_forward"), step_names.index("report_real_training_preflight"))
        self.assertLess(step_names.index("report_real_training_preflight"), step_names.index("train"))
        self.assertLess(step_names.index("evaluate"), step_names.index("optional_attach_occ3d_labels"))
        self.assertLess(step_names.index("optional_attach_occ3d_labels"), step_names.index("validate_public_occupancy_manifest"))
        self.assertLess(step_names.index("validate_public_occupancy_manifest"), step_names.index("validate_occupancy_labels"))
        self.assertLess(step_names.index("validate_occupancy_labels"), step_names.index("export_occupancy_predictions"))
        ...
        self.assertLess(step_names.index("verify_training_artifacts"), step_names.index("report_environment"))
        self.assertLess(step_names.index("report_environment"), step_names.index("verify_real_run_evidence"))
