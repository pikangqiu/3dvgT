...
class ProjectAuditTest(unittest.TestCase):
    def test_audit_reports_core_training_project_areas(self) -> None:
        ...
        self.assertIn("scripts/verify_real_run_evidence.py", items_by_name["environment"].evidence)
        self.assertIn("scripts/report_environment.py", items_by_name["environment"].evidence)
        self.assertIn("scripts/validate_model_device.py", items_by_name["environment"].evidence)
        ...
        self.assertIn("scripts/validate_public_occupancy_manifest.py", items_by_name["benchmarks"].evidence)
    ...
    def test_audit_report_is_not_complete_real_training(self) -> None:
        ...
        self.assertIn("scripts/validate_model_device.py", " ".join(report.next_actions))
        self.assertIn("--output outputs/manifest-smoke/model_device.json --json", " ".join(report.next_actions))
        ...
        self.assertIn("scripts/verify_real_run_evidence.py", " ".join(report.next_actions))
        self.assertIn("--model-device-report outputs/manifest-smoke/model_device.json", " ".join(report.next_actions))
        self.assertIn("scripts/report_environment.py", " ".join(report.next_actions))
