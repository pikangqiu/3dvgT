...
def audit_project_files(root: Path = Path(".")) -> ProjectAuditReport:
    ...
    item_specs = {
        ...
        "environment": (
            ...
            "src/vggt_project/model_device_validation.py",
    ...
    remaining_gaps = (
        ...
        "dense G3T/VGGT reference depth, pointmap, and pose target materialization, LiDAR-derived occupancy proxy generation, Occ3D label attachment, public occupancy manifest split validation, occupancy label class-id validation, occupancy prediction export, semantic occupancy metric evaluation, evaluator-output artifact validation, and model/device evidence capture are scripted, but real public checkpoint/GPU execution plus actual-data evaluator execution are not complete yet",
